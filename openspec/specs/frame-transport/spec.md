...
Define the platform-neutral frame transport vocabulary — the shared value types
(`Frame`, `PixelHandle`, `FrameMetadata`, `Lane`, `PixelFormat`,
`BufferingPolicy`) that producers and consumers use to move captured pixel data
across module and process boundaries without coupling consumers to iOS-only
capture APIs.
