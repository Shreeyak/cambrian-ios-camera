# frame-delivery Specification

## Purpose

Define how CameraKit delivers captured frames to consumers: per-lane
`AsyncThrowingStream<Frame>` subscriptions with per-lane buffering policy, the
`.primary`/`.tracker` lane vocabulary, terminal-vs-transient stream termination,
lease-returning pixel borrows, consumer-specified tracker resolution, and the
