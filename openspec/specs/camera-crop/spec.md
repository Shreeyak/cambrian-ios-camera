# camera-crop Specification

## Purpose

Define CameraKit's configurable region-of-interest crop: disabled by default with
output at the full capture resolution, a remembered default crop size
(`Constants.cropDefault*`, 1440×1440), a center-relative `setCenterCrop` API,
enable/disable without re-specifying geometry, and crop-on-open for
always-cropped consumers. Also specifies capture-resolution validation against
`SessionCapabilities.supportedSizes` and the uniform crop geometry invariants
(even origin/extents for 4:2:0 chroma alignment, fully in-bounds) enforced
