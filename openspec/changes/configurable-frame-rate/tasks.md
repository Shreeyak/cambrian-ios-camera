...
- [ ] 8.1 Logic tests: largest-4:3 default is computed and fps-independent; `(resolution, fps)` validation accepts supported and rejects unsupported pairs naming the valid set; exposure > `1/targetFps` throws; 420f invariant throws when absent (via a fake with no 420f format).
- [ ] 8.2 Device tests (iPad): open at 30 and 60 and confirm the active frame rate is locked in both preview and recording; nil-resolution default lands on the largest 4:3 (4032×3024 on the test iPad); a 60-fps request at a 30-fps-only resolution throws; a slow-mo fps is accepted where a binned format supports it.
