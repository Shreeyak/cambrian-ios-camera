...
- [x] 8.4 Docs: capture-format consumer guide (configurable/locked frame rate, exposure↔fps coupling with the reject-not-clamp contract, always-420f, HDR-off, computed largest-4:3 default) and a migration note for the exposure behavior change + default-resolution jump. — New DocC guide `11-capture-format.md` (placed after getting-started via index array order, no renumber); guide 06 stale spots fixed + cross-linked; `FrameRateRange` classified in cluster_map; `Documentation/` regenerated (also picks up prior calibration-API doc drift).
