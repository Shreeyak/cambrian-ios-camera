...
**Task 7 As-built (deviations from the literal three-site bullets):**
- **The label-publish folded into `reconcile`, not just `notifyScenePhasePaused`** (spec line 243–246 — "folds into the routine"). `reconcile` publishes the label at its top, before the suspending `.background` steps: `.active → .streaming`, `.inactive`/`.background → .paused`. `notifyScenePhasePaused` is now a thin wrapper over the same `publishCommandLabel`; both share the `shouldDeferCommandLabel` guard. This is what makes `commandLabelDefersUnderOSOwnership` non-vacuous.
- **`open()` step 10 (`publishState(.streaming)`) was removed** — a *fourth* actuation of "label folds into the routine" not in the bullet list. `reconcile` (step 9b) now owns the post-open label, and `open()` reaches it at `.closed` (no `.opening` transition inside `open()`), so the phase-correct label publishes cleanly and the `.opening → .paused` defer rider does **not** bite there. Keeping step 10 would clobber a gated launch's `.paused` back to `.streaming`. **Behavior change:** a `.background`/`.inactive` launch now publishes `.paused` (label matches gate-closed), closing the documented label-vs-gate "known gap" (`docs/ios-camera-lifecycle.md` §7) for the launch case.
- **F2 guard placement:** `osOwnsDevice` gates `startSessionIfNeeded()` + `armWatchdogs()` in `.active` and `startSessionIfNeeded()` in `.inactive`. The gate (`setGate`) is **not** guarded — `gate open ⟺ .active` holds regardless of OS state (frames are OS-stopped, so an open gate has nothing to submit).
- **New test seam `_setStateForTest(_:)`** — drives the machine to `.opening` (no engine command publishes `.opening`), the only way to exercise the `.opening → .paused` rider at the engine level.
- **Deferred HITL (Task 13):** a real-`open()`-into-`.background` publishing `.paused` is not unit-testable — `_markOpenForTest` pokes `.streaming` directly and never runs the new `open()` label path. Verify on device.
...
- [ ] **Step 3: Implement** — route the `.otherInterruptionEnded` resolve through `reconcile` (reading `currentPhase`) instead of the inline `publishState(.streaming)` + `armWatchdogs()`. This makes `reconcile`'s caller list three (update the count in any in-code comment that enumerates callers).
  - **Ordering caveat (Task 7 interaction — do NOT replace naively):** at `.otherInterruptionEnded` the machine is still `.interrupted`/`.recovering`, so `reconcile`'s own `publishCommandLabel(.streaming)` would hit `osOwnsDevice` and **defer** — leaving the terminal label stuck at `.interrupted` and breaking the relocated `interruptionEndStillRestoresStreaming`. Sequence must be: **first** clear the OS-owned state with `publishState(.streaming, kind: .event)` (the OS's authoritative "interruption ended" — `.event`, not `.command`), **then** `await reconcile()` so `osOwnsDevice` is already false and the phase-correct label/start/arm apply without deferral. Confirm `interruptionEndStillRestoresStreaming` and `interruptionEndedRearmsWatchdog` stay green after the reroute.
...
**Task 8 As-built (divergence from the plan author's stay-green expectation):**
- **Handler sequence (the ordering caveat, now implemented):** `.otherInterruptionEnded` does `publishState(.streaming, kind: .event)` (clears the OS-owned state so `osOwnsDevice` is false) **then** `await reconcile()`. A bare `reconcile()` would leave the label stuck at `.interrupted` (Task 7's deferral guard). reconcile's doc caller-list updated to three.
- **`interruptionEndedWhileBackgroundedDoesNotRearm` was migrated, not kept verbatim.** It backgrounded via a direct `setGate(false)` (which leaves `currentPhase == .active`); routing the exit through reconcile-against-`currentPhase` would then reopen the gate + re-arm. Changed to `setLifecyclePhase(.background)` (the new-model backgrounding) and its terminal-label assertion `.streaming → .paused`. **The crash-regression intent is preserved and strengthened** (watchdog-stays-disarmed + clock-advance-no-spurious-recovery both survive, now over reconcile-against-`.background`, not just the gate-guard-on-arm). The old test pinned the label-vs-gate "Known gap" (`.streaming` while gate closed) as an invariant; Task 7's reconcile-owned label **closes** that gap (backgrounded interruption-ended now settles at `.paused`), so the assertion had to invert. Advisor-confirmed this is forced by the spec (reconcile is the *same* routine across all three sites — no per-site label exception).
- **The two new tests assert end-state, not the literal stopped→running flip** for the `.inactive` row (unobservable without a synthetic seam; the `.background`-stopped vs `.inactive`-running contrast is the meaningful proof). Recording-finalize-before-stop on the `.background` exit needs a live writer — **deferred to Task 13 HITL**.
