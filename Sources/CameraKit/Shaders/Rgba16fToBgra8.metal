...
// Pass-7 — convert a half-float RGBA texture to an 8-bit BGRA IOSurface view.
// Metal's BGRA8Unorm format handles the byte-order swizzle on write; the kernel
// writes `float4(R, G, B, A)` in source channel order and the GPU stores it as
