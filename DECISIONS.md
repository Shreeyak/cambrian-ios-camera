...
2026-05-21 [lifecycle T2] coordinator — Consolidated all lifecycle/interruption tests into one `@Suite struct LifecycleTests` (`CameraKit/Tests/CameraKitTests/LifecycleTests.swift`), per user direction to keep the cross-cutting lifecycle concern in one topical suite. Relocated verbatim: `HitlLifecycleTests` (6 tests, was Stage09Tests.swift) + `ManualClock` (its sole user) + `Stage13Phase2InterruptedStateTests` (1) + `Stage13Phase2ScenePhaseMirrorGuardTests` (4, incl. its private `makeInterruptedEngine` helper) = 11 tests; their old suites removed from Stage09Tests.swift / Stage13Phase2Tests.swift. Pure move, no behavior change: device run is 18/18 green (11 LifecycleTests + 7 SessionStateMachineTests), matching the pre-move baseline. The plan's predicted `scenePhaseActiveFromInterruptedIsSkipped` name collision did not exist (stale plan note — that method lives only in the mirror-guard suite); no rename needed. Canonical filter: `eva-swift-stitchTests/LifecycleTests`. Per `docs/superpowers/plans/2026-05-21-camerakit-lifecycle-ownership.md` Task 2.

2026-05-22 [lifecycle refactor] coordinator — Extracted the App-lifecycle reconciliation cluster from `CameraEngine.swift` into a new `CameraEngine+Lifecycle.swift` (`extension CameraEngine`) — the repo's first `Type+Feature.swift` split (every other source file is flat concept-named; the convention is documented in the new file's header). Moved 10 methods: `setLifecyclePhase` (public), `reconcile` (internal — called by `open()` and `onSessionEvent`), the legacy `backgroundSuspend` / `backgroundResume` / `notifyScenePhasePaused` (internal), and 5 cluster-private helpers (`startSessionIfNeeded`, `osOwnsDevice`, `shouldDeferCommandLabel`, `publishCommandLabel`, `backgroundReconcileParkForTest`). Boundary chosen: host-intent reconciliation (moved) vs OS-event dispatch (`onSessionEvent` + `RecoveryCoordinator`/watchdog handling, kept in `CameraEngine.swift`), which is why `reconcile` is `internal` while its helpers stay `private` to the new file. Swift's cross-file access rule forced 18 members `private → internal` (15 stored/computed properties that cannot live in an extension, plus the `finalizeActiveRecording` / `armWatchdogs` / `publishState` helpers the moved code calls); all widening is intra-module — the public API is unchanged (CONTRACTS public section identical, `setLifecyclePhase` stays `public`). The "only the reconciliation path mutates phase/generation" invariant is now convention-enforced (commented on the lifecycle state block) rather than `private`-enforced. Also corrected the `setLifecyclePhase` Flutter docstring's lifecycle mapping to native UIScene callbacks (`sceneDidBecomeActive → .active`, `sceneWillResignActive → .inactive`, `sceneDidEnterBackground → .background`), matching the same `CameraKit/README.md` fix folded into this commit — `FlutterSceneLifeCycleDelegate` is the correct scene-lifecycle protocol (verified against api.flutter.dev/ios-embedder), but the events were previously the Dart `AppLifecycleState` enum names. Pure move, no behavior change: device build 0 errors / 0 warnings, 208/208 tests in 64 suites green (prior baseline), `swift-format --strict` clean. `CameraEngine.swift` 2211 → 1929 lines.

2026-05-22 [trackertex] coordinator — Tracker lane now downsamples the **processed** (graded) image, not the raw natural lane. `MetalPipeline.encode()` Pass-4 input swapped `naturalTexI → processedTexI` (the `trackerDownsample` shader already documented `processedTex` and uses a generic `inTex` param — host-only change). Rationale (user-directed): brightness/contrast/saturation/gamma/black-balance help the tracker see better too. Ordering is safe — Pass-2 writes `processedTexI` earlier in the same command buffer and Metal hazard tracking inserts the barrier, exactly as Pass-5 (NV12) and Pass-7p already rely on. Also flipped the FrameSet tracker degenerate fallback `trackerBuf ?? naturalForSet → ?? processedForSet` for semantic consistency (rare pool-exhaustion path; never reaches a `.tracker` consumer since `.tracker` only yields when `trackerBuf != nil`). The video-lane natural texture is untouched — it still feeds WB calibration (`dispatchCenterPatchOnNatural`, which requires a pre-grade signal). New test `TrackerSourceFromProcessedTests.trackerReflectsGrade` (in `RgbaConversionTests.swift`) drives a full encode with a strongly-saturated solid color + full-desaturate grade and asserts the tracker output is gray (|R-B|≈0), which only holds when sourced from the graded lane; verified RED (R=210/B=101) → GREEN on physical iPad. Existing tracker/FrameSet suites (Stage06, Stage13Phase2 FrameSet delivery, RgbaConversion tracker + end-to-end) all green on device.
