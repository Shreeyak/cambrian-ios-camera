...
/// can assert schedules without real-time waits.
final class TestClock: CameraKitClock, @unchecked Sendable {
    private let _nowMs = ManagedAtomic<UInt64>(0)
    private let lock = NSLock()
    private var _sleepRequests: [Int] = []

    func nowMs() -> UInt64 { _nowMs.load(ordering: .acquiring) }

    func sleep(milliseconds: Int) async throws {
        lock.withLock { _sleepRequests.append(milliseconds) }
        _nowMs.wrappingIncrementThenLoad(by: UInt64(milliseconds), ordering: .sequentiallyConsistent)
    }

    func advanceMs(_ d: UInt64) {
        _nowMs.wrappingIncrementThenLoad(by: d, ordering: .sequentiallyConsistent)
    }
...
struct Stage09CameraInUseTests {
    ...
    func cameraInUseSelfHealToClosed() async throws {
        ...
        #expect(states.contains(.error), "expected .error after cameraInUseBegan")
        #expect(states.last == .closed || states.contains(.closed), "expected .closed after cameraInUseEnded")
    }
}
